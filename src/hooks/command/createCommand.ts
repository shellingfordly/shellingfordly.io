...
import { RouteResult, EmptyResult, ErrorResult } from "./handleResult";
...
export function createCdCommand() {
  return (routerMap: any, value: string) => {
    ...
    if (history.path) {
      return RouteResult({ value: history.path });
    } else {
      return ErrorResult({
...
export function createLlCommand() {
  return (routerMap: any, value: string, allRoutes: RouteMap) => {
    ...
    return RouteResult({ value: _value, content });
  };
}
...
export function createFindCommand() {
  return (_: any, value: string, allRoutes: RouteMap) => {
    ...
    if (!content.length) {
      return ErrorResult({ errorType: ErrorType.NotFind, errorValue: value });
    }
